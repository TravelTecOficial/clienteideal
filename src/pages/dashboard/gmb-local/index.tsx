...
export default function GMBLocal({ className }: GMBLocalProps) {
  ...
  async function handleIdentifyGmbInLocal() {
  ...
  async function handleSearchGmbByName() {
    ...
    try {
      ...
      const candidates: PlaceCandidate[] = rawCandidates.map((c) => {
        const types = (c.types ?? []).filter((t): t is string => typeof t === "string" && !!t);
        ...
        return {
          ...
          secondaryTypes,
          location: {
            lat: typeof c.location?.lat === "number" ? c.location.lat : null,
            lng: typeof c.location?.lng === "number" ? c.location.lng : null,
          },
        };
      });
      if (!candidates.length) {
        toast({
          ...
          description: data?.message ?? "Ajuste o nome ou o endereço e tente novamente.",
        });
        return;
      }
  ...
  async function handleSelectGmbCandidate(candidate: PlaceCandidate) {
    ...
    const primaryType = candidate.primaryType?.trim() || candidate.types[0]?.trim() || null;
    if (!primaryType) {
      toast({
        variant: "destructive",
        title: "Categoria não encontrada",
              ...
              {!companyData?.gmb_place_type ? (
                <div className="p-4 rounded-lg border border-dashed border-border bg-muted/30">
                  <p className="text-sm text-muted-foreground">
                    Cadastre a categoria GMB na aba Gerenciar Perfil para listar concorrentes próximos.
                  </p>
                </div>
              ) : competitorsLoading ? (
            ...
            <CardContent className="p-6 space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Digite o nome exatamente como aparece no Google. Vamos buscar os estabelecimentos e você escolhe o
                    seu.
                  </p>
                  {companyData?.cidade && companyData?.uf && (
                    <p className="text-[11px] text-muted-foreground">
                      Cidade detectada: {companyData.cidade} / {companyData.uf}. Ajuste na aba Empresa se necessário.
                    </p>
                  )}
                </div>
                    ...
                    <div className="space-y-2 max-h-64 overflow-auto">
                      {gmbCandidates.map((c, idx) => (
                        ...
                        >
                          ...
                          {c.formattedAddress && (
                            <p className="text-[11px] text-muted-foreground mt-0.5">{c.formattedAddress}</p>
                          )}
                          {c.primaryType && (
                            <p className="text-[10px] text-muted-foreground mt-0.5">
                              Categoria: <span className="font-mono">{c.primaryType}</span>
                            </p>
                          )}
                        </button>
                      ))}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={handleIdentifyGmbInLocal}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Opcional: cole o link do seu negócio no Google Maps e clique em Identificar. A categoria será obtida
                    automaticamente e gravada.
                  </p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="gmb_place_type">Categoria GMB (identificada ou manual)</Label>
                  <Input
                    id="gmb_place_type"
