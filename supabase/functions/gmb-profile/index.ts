/**
 * Edge Function: gmb-profile
 *
 * Proxy para Business Information API v1 - get e update do perfil Google Business Profile.
 * - action "get": busca dados completos da location (title, address, phone, website, profile, hours, serviceItems)
 * - action "update": atualiza campos do perfil via locations.patch
 *
 * Requer: Authorization: Bearer <clerk_jwt>
 * Body: { action: "get" | "update", company_id?: string, updates?: Record<string, unknown> }
 *
...
async function getValidAccessToken(
  ...
  if (!isExpired) {
    try {
      ...
      console.error("[gmb-profile] Falha ao descriptografar token:", err)
  ...
  try {
    ...
    console.error("[gmb-profile] Falha ao descriptografar refresh_token:", err)
    return { error: jsonResponse({ error: "Erro ao acessar credenciais do Google." }, 500) }
  }
  ...
  if (!res.ok || !tokenData.access_token) {
    return {
      error: jsonResponse({
        error: "Erro ao renovar acesso ao Google.",
        hint: (tokenData as { error?: string }).error,
      }, 502),
    }
  }

  const newAccessToken = (tokenData.access_token as string).trim()
  const expiresIn = typeof tokenData.expires_in === "number" ? tokenData.expires_in : 3600
  const tokenExpiresAt = new Date(Date.now() + Math.max(expiresIn, 60) * 1000).toISOString()
  const newEncrypted = await encryptToken(newAccessToken, config.encryptionKey)
...
function parseBody(req: Request): Promise<RequestBody> {
  return req.json().catch(() => ({})) as Promise<RequestBody>
}
...
Deno.serve(async (req: Request) => {
  try {
    ...
    try {
      ...
      return jsonResponse({ error: "Token inválido ou expirado.", code: "INVALID_TOKEN" }, 401)
    }
    ...
    if (!companyId) {
      return jsonResponse({ error: "Empresa não identificada.", code: "NO_COMPANY" }, 400)
    }
    ...
    if (!propertyName || !propertyName.includes("/locations/")) {
      return jsonResponse({
        error: "Perfil GMB não selecionado.",
        hint: "Selecione o perfil em Configurações > Integrações.",
    ...
    if (action === "get") {
      ...
      const url = `${BI_API_BASE}/${locationPath}?readMask=${encodeURIComponent(readMask)}`
      ...
      if (!locRes.ok) {
        ...
        console.error("[gmb-profile] GET error:", locRes.status, errText)
        return jsonResponse({
          error: "Erro ao buscar perfil no Google.",
          hint: errText.slice(0, 200),
          code: "GOOGLE_API_FAILED",
        }, 502)
      }

      const location = (await locRes.json().catch(() => null)) as Record<string, unknown>
    ...
    if (!updates || typeof updates !== "object") {
      return jsonResponse({ error: "Campo updates é obrigatório para action update." }, 400)
    }
    ...
    if (updateMask.length === 0) {
      return jsonResponse({ error: "Nenhum campo válido em updates." }, 400)
    }

    const patchUrl = `${BI_API_BASE}/${locationPath}?updateMask=${encodeURIComponent(updateMask.join(","))}`
    ...
    if (!patchRes.ok) {
      ...
      console.error("[gmb-profile] PATCH error:", patchRes.status, errText)
      return jsonResponse({
        error: "Erro ao atualizar perfil no Google.",
        hint: errText.slice(0, 200),
        code: "GOOGLE_API_FAILED",
      }, 502)
    }
    ...
    console.error("[gmb-profile] Unhandled error:", err)
    return jsonResponse({
      error: "Erro interno.",
