 ...
 * Lista posts do Google Business Profile usando a API oficial do Google.
 *
 * Requer: Authorization: Bearer <clerk_jwt>
 * Query: company_id (opcional), pageSize (opcional), pageToken (opcional)
 *
 * Secrets: CLERK_SECRET_KEY, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_TOKEN_ENCRYPTION_KEY
 *
 * Deploy: npx supabase functions deploy gmb-post-list --project-ref bctjodobbsxieywgulvl --no-verify-jwt
 */
...
async function getValidAccessToken(
  ...
  if (!isExpired) {
    try {
      ...
      console.error("[gmb-post-list] Falha ao descriptografar token:", err)
  ...
  try {
    ...
    console.error("[gmb-post-list] Falha ao descriptografar refresh_token:", err)
    return { error: jsonResponse({ error: "Erro ao acessar credenciais do Google." }, 500) }
  }
  ...
  if (!res.ok) {
    console.error("[gmb-post-list] Erro ao renovar token:", res.status, tokenData)
  ...
  try {
    ...
    console.error("[gmb-post-list] Falha ao criptografar novo token:", err)
...
interface LocalPostFromApi {
  ...
  state?: string
  searchUrl?: string
  media?: Array<{ sourceUrl?: string; googleUrl?: string }>
}

function mapLocalPostToFrontend(p: LocalPostFromApi): {
  id: string
  content: string
  status: string
  ...
  const mediaUrl =
    (firstMedia?.sourceUrl ?? firstMedia?.googleUrl ?? null) ?? null

  const state = p.state ?? "LIVE"
  const status = state === "LIVE" ? "published" : state === "PROCESSING" ? "processing" : state.toLowerCase()
  ...
  return {
    ...
    scheduledAt: null,
    publishedAt: p.state === "LIVE" ? (p.createTime ?? null) : null,
...
Deno.serve(async (req: Request) => {
  ...
  if (!companyId) {
    return jsonResponse({ error: "Usuário sem empresa vinculada.", code: "NO_COMPANY" }, 403)
  }
  ...
  const listUrl = new URL(`${MYBUSINESS_API_BASE}/${locationName}/localPosts`)
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[gmb-post-list] Google API error:", res.status, errMsg)
      return jsonResponse(
        {
          error: "Erro ao listar postagens do Google Business.",
          hint: errMsg,
          code: "GOOGLE_POST_LIST_FAILED",
        },
        502,
      )
    }

    const localPosts = data.localPosts ?? []
    const posts = localPosts.map(mapLocalPostToFrontend)

    const byDate = (
      a: { publishedAt: string | null; scheduledAt: string | null; createdAt: string | null },
      b: { publishedAt: string | null; scheduledAt: string | null; createdAt: string | null },
    ) => {
