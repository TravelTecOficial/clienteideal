-- GMB Local: tabelas para Health Checks e Audit Items
-- Multitenancy: company_id em todas as tabelas. RLS garante isolamento.

-- gmb_health_checks: score e contagens por empresa
CREATE TABLE public.gmb_health_checks (
...
CREATE TABLE public.gmb_audit_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id text NOT NULL REFERENCES public.companies(id),
  category text,
...
-- Políticas SELECT (padrão: is_saas_admin ou company_id do profile)
...
CREATE INDEX idx_gmb_health_checks_company_id ON public.gmb_health_checks(company_id);
CREATE INDEX idx_gmb_audit_items_company_id ON public.gmb_audit_items(company_id);
CREATE INDEX idx_gmb_audit_items_company_ordem ON public.gmb_audit_items(company_id, ordem);

-- Seed opcional: insere dados de exemplo para a primeira empresa (se existir)
-- O frontend usa fallback (score 91, contagens, lista vazia) quando não há dados.
